            ...
            <div class="container px-4 mx-auto flex flex-wrap items-center justify-between">
                <div class="w-full relative flex items-center justify-between">
                    <router-link :to="{
                        ...
                        <div
                            class="bg-red-500 hover:bg-red-600 text-white tex-lg font-black shadow-md px-4 py-2.5 rounded flex items-center">
                        ...
                        <section aria-labelledby="products-heading" class="py-6">
                            <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                                <div :class="formatif?.current ? 'col-span-4' : 'col-span-3'"
                                    ...
                                    <section v-if="
                                        formatif?.current === null ||
                                        formatif?.current.waktuMulai_TesFormatif === null
                                    " aria-labelledby="products-heading" class="col-span-4 pb-24">
                                        <h1 class="text-xl font-medium mb-5">
                                            <span class="capitalize">
                                                Tes Formatif Sub-CPMK "{{ currentUnit?.current.narasi_subCpmk }}"
                                            </span>
                                            <div class="font-bold">Perhatian:</div>
                                            <ol v-if="
                                                kelas?.settings.batas_pengulangan_remidi -
                                                formatif?.completed.length >
                                                0
                                            " class="text-md italic font-light list-decimal ml-6">
                                                <li class="text-md" v-if="
                                                    kelas?.settings.batas_pengulangan_remidi -
                                                    formatif?.completed.length ==
                                                    kelas?.settings.batas_pengulangan_remidi
                                                ">
                                                    Semua materi pada Sub-CPMK ini sudah anda lalui, silakan mulai
                                                    mengerjakan tes formatif untuk mengukur tingkat penguasaan Anda.
                                                </li>
                                                <li class="text-md" v-if="
                                                    kelas?.settings.batas_pengulangan_remidi -
                                                    formatif?.completed.length !=
                                                    kelas?.settings.batas_pengulangan_remidi
                                                ">
                                                    Skor Anda belum memenuhi ketuntasan minimum dalam sub-CPMK ini.
                                                </li>
                                                <li class="text-md" v-if="
                                                    kelas?.settings.batas_pengulangan_remidi -
                                                    formatif?.completed.length !=
                                                    kelas?.settings.batas_pengulangan_remidi
                                                ">
                                                    Silakan Anda mengikuti tahap remidi melalui belajar kelompok
                                                    dengan teknik “Tutor Sebaya” di luar sistem.
                                                </li>
                                                <li class="text-md">
                                                ...
                                                <li v-if="
                                                    formatif?.completed.length != 0 &&
                                                    formatif?.completed.at(-1).status_TesFormatif == 2 &&
                                                    kelas.settings.waktu_tunggu_formatif > 0
                                                " class="text-md">
                                                    Anda bisa melakukan tes formatif (remidi) berikutnya setelah
                                                    <strong>{{ waktuTungguRemidi }}</strong>
                                                    Jam
                                                    <strong>{{ arrayTimeRemidi[0].split(":")[0] }}:{{
                                                        arrayTimeRemidi[0].split(":")[1]
                                                    }}</strong>
                                                    {{
                                                        `${arrayTimeRemidi[2]} ${arrayTimeRemidi[3]} ${arrayTimeRemidi[4]}`
                                                    }}
                                                </li>
                                                <li>
                                            ...
                                            <ol v-else class="text-md italic font-light list-decimal ml-6">
                                                <li class="text-md">
                                                    Skor Anda tetap belum memenuhi ketuntasan minimum dalam sub-CPMK
                                                    ini.
                                                </li>
                                                <li class="text-md">
                                                    Anda tidak memiliki kesempatan untuk mengikuti tes formatif
                                                    ulang kembali.
                                                </li>
                                                <li class="text-md">
                                                    Silakan Anda segera menghubungi dosen pengajar untuk melakukan
                                                    proses pendampingan.
                                                </li>
                                            </ol>
                                        </h1>
                                        ...
                                        " class="mt-3 flex justify-start items-center">
                                            <button @click.prevent="createFormatif()" :class="
                                                kelas.settings.waktu_tunggu_formatif > 0
                                                    ? 'bg-red-500 hover:bg-red-600'
                                                    : 'bg-emerald-500 hover:bg-emerald-600'
                                            " class="text-xl font-bold px-6 py-3 rounded text-white">
                                                Mulai Tes Formatif
                                            </button>
                                        </div>
                                        <div v-if="
                                        ...
                                        " class="mt-12">
                                            <h1 class="font-black text-lg">
                                                Teman Anda yang telah tuntas dan direkomendasikan menjadi tutor
                                                sebaya
                                            </h1>
                                            <ul>
                                                <li v-if="failedInfo?.top.length == 1">
                                                    - Belum ada Rekomendasi Mahasiswa
                                                </li>
                                                <template v-for="(item, index) in failedInfo?.top" :key="index">
                                                    <li v-if="item.email_siswa != authStore.authUser.email">
                                                        - {{ item.email_siswa }}
                                                    </li>
                                                </template>
                                            </ul>
                                        ...
                                        " class="mt-6">
                                            <h1 class="font-black text-lg">
                                                Daftar mahasiswa yang mengikuti remidi pada sub-CPMK ini
                                            </h1>
                                            <ul>
                                                <li v-if="failedInfo?.failed.length == 1">
                                                    - Belum ada Mahasiswa Remidi selain Anda
                                                </li>
                                    ...
                                    <template v-else>
                                        <div class="lg:col-span-3">
                                            <div class="mb-2 text-lg">
                                                <p class="font-bold text-xl">Petunjuk:</p>
                                                <ol class="list-decimal ml-6">
                                                    <li>
                                                        Soal yang sedang aktif adalah soal yang sedang dikerjakan.
                                                    </li>
                                                    <li>
                                                        Jika ingin beralih ke soal berikutnya, silakan pilih nomor
                                                        soal pada daftar soal di sebelah kanan.
                                                    </li>
                                                    <li>
                                                        Sebelum mengklik tombol "Selesai", Anda memiliki kesempatan
                                                        untuk mengubah jawaban. Caranya adalah dengan memilih nomor
                                                        soal di bagian kanan, yang ingin diubah jawabannya di sebelah
                                                        kanan.
                                                    </li>
                                                    <li>
                                                        Jika tombol "Selesai" telah diklik, Anda tidak memiliki
                                                        kesempatan untuk mengubah jawaban.
                                                    </li>
                                                </ol>
                                            </div>
    ...
    axios
        ...
        .catch((err) => {
            if (err.response.data.message == "current unit is not finished") {
                ...
                Swal.fire(
                    "Peringatan",
                    `Silakan baca materi terlebuh dahulu sebelum mengerjakan tes formatif`,
                    "warning"
                )
                ...
                Swal.fire(
                    "Peringatan",
                    `Anda belum bisa melakukan tes formatif ulang, Silakan tunggu hingga waktu yang ditentukan`,
                    "warning"
                )
            }
        })
...
const startFormatif = () => {
    axios
        ...
        .catch((res) => {
            ...
            Swal.fire(
                "Skor Anda tetap belum memenuhi ketuntasan minimum dalam sub-CPMK ini",
                "Anda tidak memiliki kesempatan untuk mengikuti tes formatif ulang kembali. Silakan Anda segera menghubungi dosen pengajar untuk melakukan proses pendampingan",
                "warning"
            )
        })
}
...
watch(formatif, () => {
    if (
        formatif.value?.completed.length != 0 &&
        formatif.value?.completed.at(-1).status_TesFormatif == 2
    ) {
...
const getSoal = (no_urut_soal = 1) => {
    axios
        ...
        .then((res) => {
            ...
            let hasil = soalInfo.value?.jawaban.find(
                (jwb) => jwb.nomorUrut_soal === no_urut_soal
            )
...
const soalTimer = (usedDate, countSoal) => {
    ...
    if (usedDate !== undefined) {
        ...
        end.getMinutes() + kelas.value?.settings.waktu_per_soal_formatif * countSoal
    )
    let timer = setInterval(() => {
        ...
        let timeStampCountDown = Math.floor(
            new Date(end.getTime() - utcnow.getTime()).getTime() / 1000
        )

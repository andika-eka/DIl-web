                            ...
                            <div class="fixed inset-0 z-40 flex">
                                <TransitionChild as="template" enter="transition ease-in-out duration-300 transform"
                                    ...
                                    <DialogPanel
                                        ...
                                        <Disclosure as="div" v-for="(subCPMK, subCPMKidx) in matakuliah?.sub_cpmk"
                                            :key="subCPMKidx" class="border-b border-gray-200 py-6 ml-6" v-slot="{ open }">
                                            <h3 class="-my-3 flow-root pr-6">
                                                <DisclosureButton :class="
                                                    subCPMK?.id_subCpmk == selectedMateri?.id_subCPMK
                                                        ? 'font-bold'
                                                        : ''
                                                "
                                                    class="flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500">
                                                    <span class="text-left text-gray-900 capitalize">{{
                                                        subCPMK.narasi_subCpmk
                                                    }}</span>
                                            ...
                                            <DisclosurePanel class="pt-6">
                                                <div class="space-y-4">
                                                    <Disclosure as="div"
                                                        v-for="(indikator, indikatorIdx) in subCPMK?.indikator"
                                                        :key="indikatorIdx" class="border-b border-gray-200 py-6 ml-6"
                                                        v-slot="{ open }">
                                                        <h3 class="-my-3 flex items-center">
                                                            <div v-if="
                                                                subCPMK?.id_subCpmk > currentUnit?.current.id_subCpmk ||
                                                                indikator.id_indikator > currentMateri?.id_indikator
                                                            ">
                                                            ...
                                                            <DisclosureButton :class="
                                                                indikator?.id_indikator ==
                                                                    selectedMateri.currentMateri?.id_indikator
                                                                    ? 'font-bold'
                                                                    : ''
                                                            "
                                                                class="flex w-full items-center justify-between bg-white py-3 text-sm text-gray-400 hover:text-gray-500">
                                                                <span class="text-left text-gray-900 capitalize">{{
                                                                    indikator.narasi_indikator
                                                                }}</span>
                                                                <span class="ml-6 flex items-center">
                                                        ...
                                                        <DisclosurePanel class="pt-6">
                                                            <div class="space-y-4">
                                                                <div v-for="(materi, materiIdx) in indikator?.materi"
                                                                    :key="materiIdx"
                                                                    class="flex items-center select-none cursor-pointer ml-6">
                                                                    <template v-if="
                                                                        indikator?.id_subCpmk <=
                                                                        currentUnit?.current.id_subCpmk
                                                                    ">
                                                                        <div v-if="
                                                                            materi.id_materi <= currentMateri?.id_materi ||
                                                                            currentMateri == undefined
                                                                        ">
                                                                    ...
                                                                    " :class="
    materi?.id_materi ==
        selectedMateri.currentMateri?.id_materi
...
" class="ml-3 text-sm text-gray-600 capitalize">
                                                    ...
                                                    <DisclosurePanel class="pt-6 ml-6">
                                                        <div class="space-y-4">
                                                            <div @click="gotoTesFormatif(subCPMKidx)"
                                                                class="flex items-center select-none cursor-pointer">
                                                                <div>
                                                                    <CheckCircleIcon
                                                                        v-if="currentUnit?.completed[subCPMKidx]"
                                                                        class="w-5" />
                            ...
                            <div class="grid grid-cols-1 gap-x-8 gap-y-10 lg:grid-cols-4">
                                <div class="lg:col-span-3">
                                    ...
                                    <template v-if="materiExits">
                                        <div>
                                            <iframe lazy width="100%" height="480" allow="autoplay" frameborder="0"
                                                allowfullscreen :src="
                                                    parseYoutubeLink(selectedMateri.currentMateri?.pathFile_materi)
                                                "></iframe>
                                        </div>
                                    </template>
                                    <template v-else>
                                        <div v-if="
                                            sesiFormatif == false &&
                                            currentUnit?.current.status_subcpmkpengambilan != 3
                                        " class="bg-red-100 px-6 py-3 rounded text-gray-600 flex items-center">
                                            <div class="mr-6">
                                        ...
                                        <template v-else>
                                            <template v-if="
                                                currentUnit?.completed.length === matakuliah?.sub_cpmk.length &&
                                                formatif?.completed.at(-1).status_TesFormatif == 3
                                            ">
                                                <div
                                                    class="bg-green-50 shadow px-6 py-4 rounded text-gray-600 flex items-center">
                                                    <CheckCircleIcon
                                                        class="h-10 w-10 top-4 left-3 text-4xl text-emerald-500 opacity-50" />
                                                    <div class="ml-10 relative">
                                                        <p class="text-lg font-light italic">
                                                            Semua materi Sub-CPMK sudah Anda selesaikan, Tes Sumatif
                                                            akan diadakan serentak pada
                                                            <strong>{{
                                                                kelas?.settings.tgl_sumatif
                                                                ? new Date(kelas?.settings.tgl_sumatif).toDateString()
                                            ...
                                            <template v-else>
                                                <div
                                                    ...
                                                    <div class="ml-10 relative">
                                                        <p class="text-lg font-light italic">
                                                            Semua materi sudah anda selesaikan, anda sudah bisa
                                                            mengambil tes formatif
                                                        </p>
                                                    </div>
                                                </div>
                                                <router-link :to="{
                                    ...
                                    <div v-if="
                                        selectedMateri.currentMateri?.id_indikator ==
                                        currentMateri?.id_indikator && materiExits
                                    " class="mt-6">
                                        <button @click.prevent="nextMateri()"
                                            class="px-6 py-3 bg-emerald-500 hover:bg-emerald-600 rounded text-white">
                                            Materi Selanjutnya
                                        </button>
                                    </div>
                                        ...
                                        <div class="border-t border-gray-200">
                                            <dl>
                                                ...
                                                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 capitalize">
                                                        Mata kuliah basis data mempelajari teknik-teknik dasar dalam
                                                        membangun basis data. Adapun jenis basis data yang dipelajari
                                                        pada mata kuliah ini adalah model basis data relasional.
                                                        Terkait dengan hal tersebut maka dalam mata kuliah ini
                                                        diberikan beberapa konsep tentang basis data, sistem basis
                                                        data, dan model basis data. Beberapa teknik dalam perancangan
                                                        basis data juga diberikan, seperti perancangan basis data
                                                        menggunakan Diagram Entity-Relationship (Diagram E-R).
                                                        Mentransformasikan Diagram E-R menjadi Basis Data Relasional.
                                                        Melakukan normalisasi basis data untuk mengatasi masalah
                                                        anomali data (1NF-3NF). Materi terakhir adalah membuat
                                                        rancangan basis data untuk studi kasus ssitem informasi
                                                ...
                                                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 capitalize">
                                                        <span>
                                                            {{
                                                                selectedMateri.sub_cpmk?.narasi_subCpmk
                                                                ? selectedMateri.sub_cpmk?.narasi_subCpmk
                                                                : currentUnit?.current.narasi_subCpmk
                                                            }}
                                                        </span>
                                                        <ul v-if="selectedMateri.sub_cpmk?.narasi_subCpmk"
                                                            class="ml-6 list-disc">
                                                            <li v-for="(indikator, indikatorIdx) in selectedMateri
                                                                .sub_cpmk?.indikator" :key="indikatorIdx">
                                                                {{ indikator.narasi_indikator }}
                                                            </li>
                                                        </ul>
                                                        <ul v-else class="ml-6 list-disc">
                                                            <li v-for="(indikator, indikatorIdx) in materiList?.indikator"
                                                                :key="indikatorIdx">
                                                                {{ indikator.narasi_indikator }}
                                                            </li>
                                                        </ul>
                                                ...
                                                <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                                    ...
                                                    <dd class="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0">
                                                        <a :href="
                                                            selectedMateri.sub_cpmk?.pathFile_materiTeks
                                                                ? parseMateriTeks(
                                                                    selectedMateri.sub_cpmk?.pathFile_materiTeks
                                                                )
                                                                : parseMateriTeks(
                                                                    currentUnit?.current.pathFile_materiTeks
                                                                )
                                                        " download=""
                                                            class="flex bg-emerald-500 hover:bg-emerald-600 px-4 py-2 rounded"
                                                            target="_blank">
                                                            <PaperClipIcon class="h-5 w-5 flex-shrink-0 text-white"
                                                                aria-hidden="true" />
                                ...
                                <form class="hidden lg:block">
                                    ...
                                    <Disclosure as="div" v-for="(subCPMK, subCPMKidx) in matakuliah?.sub_cpmk"
                                        ...
                                        <DisclosurePanel class="pt-6">
                                            <div class="space-y-4">
                                                <Disclosure as="div" v-for="(indikator, indikatorIdx) in subCPMK?.indikator"
                                                    :key="indikatorIdx" class="border-b border-gray-200 py-6 ml-6"
                                                    v-slot="{ open }">
                                                    <h3 class="-my-3 flex items-center">
                                                        <div>
                                                    ...
                                                    <DisclosurePanel class="pt-6">
                                                        <div class="space-y-4">
                                                            <div v-for="(materi, materiIdx) in indikator?.materi"
                                                                ...
                                                                " class="ml-3 text-sm capitalize"
                                                                    :class="materi?.id_materi == selectedMateri.currentMateri?.id_materi ? 'font-bold text-sm text-emerald-500' : 'text-gray-600'">
...
<script setup>
import {
    Dialog, DialogPanel, Disclosure, DisclosureButton, DisclosurePanel, Menu,
...
import {
    onBeforeUnmount, onMounted, onUnmounted, onUpdated,
    ...
    axios
        ...
        .catch((err) => {
            ...
            if (
                ...
                err.response.data.message == "siswa has to go to test"
            ) {
...
const goToMateri = (id_matakuliah, indikatorIdx, dataMateri) => {
    if (id_matakuliah <= currentUnit.value?.current.id_subCpmk) {
        if (
            dataMateri.id_materi <= currentMateri.value?.id_materi ||
            currentMateri.value == undefined
        ) {
            ...
            Swal.fire(
                "Materi Non-Aktif",
                "<p>Silakan lanjutkan belajar anda, pada materi terakhir yang terbuka</p>",
                "warning"
            )
        }
    } else {

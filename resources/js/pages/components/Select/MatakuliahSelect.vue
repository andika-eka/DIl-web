...
const getMatakuliah = async () => {
    await authAxios.get("/api/Matakuliah").then((res) => {
        matakuliah.value = res.data;
    });
};

watch(selected, () => {
    kelasStore.kelas.id_matakuliah = selected.value.id_matakuliah;
});
onMounted(async () => {
    await getMatakuliah();
});
